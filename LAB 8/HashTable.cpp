...
void HashTable::put(const string &s, int score) {
    ...
    if (contains(s)) {
        //ADD NEW APPEARANCE TO WORD W/ SCORE AS PARAMETER
        for (auto i = hashTable[index].begin(); i != hashTable[index].end(); ++i) {
            if (i->getWord() == s) {i->addNewAppearance(score);}
        }
        return;
    }

    WordEntry newEntry(s, score);   //creates new entry with parameters
    hashTable[index].push_back(newEntry);       //pushes the entry into the proper list in hashtable
